...
class CustomerPaymentProvider with ChangeNotifier {
  ...
  Future<String> fetchNextPaymentNumber() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('token');

      // Always fetch ALL payments (no salesman filter) for correct numbering
      final response = await http.get(
        Uri.parse('${ApiEndpoints.baseUrl}/customer-payments'),
        headers: {
          'Authorization': 'Bearer $token',
      ...
      if (response.statusCode == 200) {
        final jsonData = jsonDecode(response.body);
        final allPayments = CustomerPaymentModel.fromJson(jsonData).data.payments;

        if (allPayments.isEmpty) return 'CP-0001';

        final allNumbers = allPayments.map((p) {
          final parts = p.paymentNo.split('-');
          return int.tryParse(parts.last) ?? 0;
        }).toList();

        final maxNumber = allNumbers.reduce((a, b) => a > b ? a : b);
        return 'CP-${(maxNumber + 1).toString().padLeft(4, '0')}';
      }
    } catch (e) {
      debugPrint('Error fetching CP number: $e');
    }
    return 'CP-0001'; // fallback
  }
}