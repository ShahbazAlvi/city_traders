...
class _CategoriesDropdownState extends State<CategoriesDropdown>
  ...
  Future<void> _openSearchSheet(List<CategoriesModel> categories) async {
    final result = await showModalBottomSheet<CategoriesModel>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) =>
          _CategoriesSearchSheet(categories: categories, label: widget.label),
    );

    if (result != null) {
      setState(() => selectedCategory = result);
      widget.onChanged(result.id.toString());
  ...
  Widget build(BuildContext context) {
    return Consumer<CategoriesProvider>(
      builder: (context, provider, child) {
        ...
        final categories = List<CategoriesModel>.from(provider.categories);
        categories.sort((a, b) => (a.name ?? "")
            .toLowerCase()
            .compareTo((b.name ?? "").toLowerCase()));

        // Set initial selection if provided
        if (widget.selectedId != null && selectedCategory == null) {
          try {
            selectedCategory = categories.firstWhere(
              (c) => c.id.toString() == widget.selectedId,
            );
            WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        return Column(
          ...
          children: [
            GestureDetector(
              onTap: () => _openSearchSheet(categories),
              child: Container(
...
class _CategoriesSearchSheet extends StatefulWidget {
  final List<CategoriesModel> categories;
  final String label;
  const _CategoriesSearchSheet({required this.categories, required this.label});

  @override
  State<_CategoriesSearchSheet> createState() => _CategoriesSearchSheetState();
}

class _CategoriesSearchSheetState extends State<_CategoriesSearchSheet> {
  final TextEditingController _searchController = TextEditingController();
  List<CategoriesModel> _filtered = [];
                          ...
                          child: Row(
                            children: [
                              Container(
                                ...
                                height: 40,
                                decoration: BoxDecoration(
                                  color: Colors.primaries[(category.id ?? 0)
                                          .toString()
                                          .hashCode %
